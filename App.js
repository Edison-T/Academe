...
import { setServerData } from './redux/features/serverSlice';
import { setChannelData } from './redux/features/channelSlice';
...
const Tab = createBottomTabNavigator();
  ...
  const fetchUserInfo = async (userID, email) => {
    try {
      ...
      dispatch(setServerData({
        server: userData.data.getUser.serverUsers.items.map((object) => object.server)
      }));
      // dispatch(setChannelData({
      //   channel: (userData.data.getUser.serverUsers.items.map((object) => object.server).map((object) => object.categories.items))
      // }))
  } catch (e) {
      console.log(e);
      return
      ...
      switch (event) {
        case "signIn":
          try {
            ...
            checkForExistingUser(userID).then(() => fetchUserInfo(userID, email)).catch((e) => console.log(e))
          }
          catch (e) {
            console.log(e)
          }
