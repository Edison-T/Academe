...
import { setServerData, logout, selectServer } from '../../redux/features/serverSlice';
...
const DrawerNavigatorComponent = ({route}) => {
    ...
    const serverData = useSelector(selectServer)

    const servers = [
        {
            serverIconName: "school-outline" as const,
            //must use optional chaining operator '?' because the render is too fast for the redux data fetch.
            data: serverData.server?.filter((server) => server.typeOfServer === "Class") || []
        },
        {
            serverIconName: "people-outline" as const,
            data: serverData.server?.filter((server) => server.typeOfServer === "Group") || []
        },
        {
            serverIconName: "planet-outline" as const,
            data: serverData.server?.filter((server) => server.typeOfServer === "Public") || []
        }
    ];
